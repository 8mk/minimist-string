...
module.exports = function (input) {
	...
	if (input.includes('"') || input.includes("'")) {
		...
		var countQuotes = function (piece, quote_char) {
			...
			return replaced
				.replace(new RegExp("(\\\\" + quote_char + ")", "g"), "")
				.replace(/\\/g, "").length;
		};
		...
		var splitPiece = function (piece, quote_char) {
			...
			var second_q_index = getFirstQuote(
				piece,
				quote_char,
				first_q_index + 1
			);

			var first_part = piece.substring(0, second_q_index + 1);
			var second_part = piece.substring(second_q_index + 1, piece.length);
		...
		for (var i = 0; i < good_pieces.length; i++) {
			...
			good_pieces[i] = good_pieces[i].replace(
				/(\\\')/g,
				"%%%SINGLEQUOTE%%%"
			);
			good_pieces[i] = good_pieces[i].replace(
		...
		argv = minimist(input.replace(this.prefix + " ", "").split(" "));
	}
